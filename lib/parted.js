...
// minimal buffering (only 4 bytes)
...
Parser.prototype.write = function(data) {
  ...
  for (; i < l; i++) {
    ...
    switch (this.state) {
      ...
      case 'IN_HEADER_VAL':
        if (data[i] === LF && this.last[0] === CR) {
          // trim off the single leading space thats usually in headers
          // do -1 on the slice because theres no sane way to exclude the CR
          this.headers[this.hname] = this.hval; //this.hval.slice(0, -1).trim();
          this.hname = this.hval = '';
          this.state = 'END_OF_HEADER';
        } else {
          if (!((this.hval.length === 0 && data[i] === SPACE) 
            || data[i] === CR)) {
...
Parser.prototype._finish = function() {
  //this.parts[this.field] = {
  //  name: this.field,
  //  filename: this.filename,
  //  type: this.type,
  //  encoding: this.encoding,
  //  data: this.data.path || this.data
  //};
...
exports.middleware = function(opt) {
  ...
  return function(req, res) {
    ...
    if (type && type.indexOf('multipart/form-data') !== -1) {
      Parser(req, next, opt);
      //req.pipe(new Parser().on('end', next).on('error', next));
    } else {
      next();
    }
  };
