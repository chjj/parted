...
var parted = function(type, opt) {
  ...
  // this buffer is not a lookbehind. 
  // it is here to buffer header 
  // names and values.
...
parted.prototype.write = function(data) {
  if (this.done) {
    // this parser assumes a message is finished after
    // the second dash of the last boundary key
    // however, some browsers send another CRLF.
    // there might be 2 extra bytes at the end
    // that we can ignore.
    ...
    switch (this.state) {
      ...
      case 'maybe_in_key':
        if (this.pos === key.length - 1) {
          ...
          if (!this.verified) {
            return this._error('Parse error1.');
          }
          this.state = 'in_body';
      ...
      case 'in_key_end':
        switch (ch) {
          ...
          case DASH:
            ...
            break;
          default: 
            return this._error('Parse error2.');
        }
        break;
      case 'in_key_line_end':
        switch (ch) {
          case LF:
            ...
            break;
          default: 
            return this._error('Parse error3.');
        }
        break;
      case 'in_key_dash_end':
        switch (ch) {
          case DASH:
            ...
            return;
          default: 
            return this._error('Parse error4.');
        }
        break;
      case 'in_header_name':
        switch (ch) {
      ...
      case 'in_header_val_cr':
        switch (ch) {
          case LF:
            ...
            break;
          default: 
            return this._error('Parse error5.');
        }
        break;
      case 'in_header_end':
        switch (ch) {
      ...
      case 'in_head_end_cr':
        switch (ch) {
          case LF:
            ...
            break;
          default:
            return this._error('Parse error6.');
        }
        break;
      case 'in_body':
        switch (ch) {
