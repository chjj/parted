...
var Parser = function(type, options) {
  ...
  this.options.encoding = this.options.encoding || 'ascii';
  ...
  for (; i < len; i++) {
    ...
    switch (this.state) {
      ...
      case 'header_name':
        switch (ch) {
          case COLON:
            this.header = buff.toString(this.options.encoding, 0, this.pos);
            this.pos = 0;
      ...
      case 'header_val_end':
        switch (ch) {
          case LF:
            val = buff.toString(this.options.encoding, 0, this.pos);
...
Parser.handle = function(req, res, next, options) {
  ...
  parser.on('data', function() {
    ...
    if (this.writtenDisk > diskLimit) {
      err = new Error('Multipart disk overflow')
      if (typeof options.handleMultipartDiskOverflow === "function") {
        options.handleMultipartDiskOverflow(err, req, res, next);
        err.hasCustomHandler = true;
        throw err;
      }
    } else if (this.written > limit) {
      err = new Error('Multipart field size overflow');
      if (typeof options.handleMultipartMaxFieldsSizeOverflow === "function") {
        options.handleMultipartMaxFieldsSizeOverflow(err, req, res,next );
        err.hasCustomHandler = true;
        throw err;
      }
    }
...
// This implementation was taken from formidable's module for parsing form data from file uploads
// https://github.com/node-formidable/formidable
var _getFilename = function (headerValue) {
