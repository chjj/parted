...
var grab = function(str, name) {
  ...
  const STRING = 0, NUMBER = 1, DETECT = 2, END = 3;
  var start = str.indexOf(name+'='), value = '';
  ...
  for(var pos = start, len = str.length, state = DETECT, type, tmp; pos <= len; pos++)
    if(state === DETECT) {
      state = null;
      if(str[pos] === '"') 
        type = STRING;
      else {
        tmp = parseInt(str[pos]);
        if(!Number.isNaN(tmp) ) 
          type = NUMBER;
        else {
          type = STRING;
    ...
    else if( pos === len || ( ( str[pos] === ';' || str[pos] === '"') && str[pos-1] !== '\\') )
      return type === NUMBER ? parseInt(value, 10) : (value.length ? value.substr(0, pos - start) : undefined);
    else
       value+=str[pos];
